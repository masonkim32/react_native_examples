...
class ContinueButton extends Component {
  render() {
    let text = this.props.wasCorrect ?
      "Correct! Next card?" : "Oops, not quite. Next card?";
    return (
...
function mkContinueQuitButtons(showingAnswer, wasCorrect, continueFunc, quitFunc) {
  if (showingAnswer) {
    return <ContinueButton opPress={continueFunc} wasCorrect={wasCorrect} />
  } else {
    return (
...
function mkAnswerButtons(answers, correctAnswer, showingAnswer, wasCorrect, selectAnswerFunc) {
  if (!answers) {
    return null;
  }
